...
install_grub_signed_shim() {
    ...
    # Determine disk and partition for efibootmgr
    # boot_partition is like /dev/sda1 or /dev/nvme0n1p1
    local disk_name
    disk_name=$( lsblk -no pkname "$boot_partition" )
    local disk_path="/dev/$disk_name"
    local part_num
    part_num=$( lsblk -no PARTN "$boot_partition" )

    arch-chroot "$INSTALL_POINT" /bin/bash <<EOF
    pacman -S --noconfirm grub efibootmgr os-prober shim-signed
    ...
    # Install GRUB
    # We install to MYKYcorp_GRUB directory
    grub-install \
      --target=x86_64-efi \
      --efi-directory=/boot \
      --bootloader-id=MYKYcorp_GRUB \
    ...
    # Shim Setup
    echo "Setting up Shim..."
    # Copy shim and MokManager
    cp /usr/share/shim-signed/shimx64.efi /boot/EFI/MYKYcorp_GRUB/shimx64.efi
    cp /usr/share/shim-signed/mmx64.efi /boot/EFI/MYKYcorp_GRUB/mmx64.efi

    # Handle Removable vs Non-Removable
    if [ "$grubstate" == "--removable" ]; then
        echo "Configuring for removable media (fallback path)..."
        # grub-install --removable installs to /boot/EFI/BOOT/BOOTX64.EFI
        # We need to overwrite it with Shim, and rename Grub to GRUBX64.EFI
        
        # Check if BOOTX64.EFI exists (it should, from grub-install)
        ...
        # Register Shim with efibootmgr
        echo "Registering Shim with efibootmgr..."
        
        efibootmgr --create --disk "$disk_path" --part "$part_num" --loader /EFI/MYKYcorp_GRUB/shimx64.efi --label "MYKYcorp (Secure Boot)" --verbose
    fi
